...
/*******************************************************************************
* Function Name: Update_CLK_Start
********************************************************************************
*
* Summary:
...
void Update_CLK_Start(void) 
{
    ...
    Update_CLK_CLKEN |= Update_CLK_CLKEN_MASK;
	Update_CLK_CLKSTBY |= Update_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Update_CLK_Stop
********************************************************************************
*
* Summary:
...
void Update_CLK_Stop(void) 
{
    ...
    Update_CLK_CLKEN &= (uint8)(~Update_CLK_CLKEN_MASK);
	Update_CLK_CLKSTBY &= (uint8)(~Update_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Update_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Update_CLK_CLKEN & Update_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Update_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Update_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Update_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Update_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Update_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Update_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Update_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Update_CLK_GetSourceRegister();
    uint16 oldDivider = Update_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Update_CLK_CLKEN & Update_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Update_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Update_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Update_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Update_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Update_CLK_MOD_SRC |= modeBitMask & (uint8)Update_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Update_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Update_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Update_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Update_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Update_CLK_MOD_SRC & (uint8)(Update_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Update_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Update_CLK_GetDividerRegister();
    uint8 oldSrc = Update_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Update_CLK_MOD_SRC |= CYCLK_SSS;
        Update_CLK_MOD_SRC =
            (Update_CLK_MOD_SRC & (uint8)(~Update_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Update_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Update_CLK_MOD_SRC & Update_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Update_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Update_CLK_PHASE = clkPhase & Update_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Update_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Update_CLK_PHASE & Update_CLK_PHASE_MASK;
}
