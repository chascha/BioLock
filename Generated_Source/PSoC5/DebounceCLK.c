...
/*******************************************************************************
* Function Name: DebounceCLK_Start
********************************************************************************
*
* Summary:
...
void DebounceCLK_Start(void) 
{
    ...
    DebounceCLK_CLKEN |= DebounceCLK_CLKEN_MASK;
	DebounceCLK_CLKSTBY |= DebounceCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DebounceCLK_Stop
********************************************************************************
*
* Summary:
...
void DebounceCLK_Stop(void) 
{
    ...
    DebounceCLK_CLKEN &= (uint8)(~DebounceCLK_CLKEN_MASK);
	DebounceCLK_CLKSTBY &= (uint8)(~DebounceCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DebounceCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DebounceCLK_CLKEN & DebounceCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DebounceCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DebounceCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DebounceCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DebounceCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DebounceCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DebounceCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DebounceCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DebounceCLK_GetSourceRegister();
    uint16 oldDivider = DebounceCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DebounceCLK_CLKEN & DebounceCLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DebounceCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: DebounceCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(DebounceCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: DebounceCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DebounceCLK_MOD_SRC |= modeBitMask & (uint8)DebounceCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: DebounceCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DebounceCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DebounceCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: DebounceCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DebounceCLK_MOD_SRC & (uint8)(DebounceCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: DebounceCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DebounceCLK_GetDividerRegister();
    uint8 oldSrc = DebounceCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DebounceCLK_MOD_SRC |= CYCLK_SSS;
        DebounceCLK_MOD_SRC =
            (DebounceCLK_MOD_SRC & (uint8)(~DebounceCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DebounceCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DebounceCLK_MOD_SRC & DebounceCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DebounceCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DebounceCLK_PHASE = clkPhase & DebounceCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DebounceCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DebounceCLK_PHASE & DebounceCLK_PHASE_MASK;
}
